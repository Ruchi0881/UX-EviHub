...
const api = {
    ...
    delete: async (url) => {
      const response = await fetch(`${baseUrl}/api${url}`, { method: 'DELETE' });
      if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
      return { data: await response.json() };
    },
  ...
  export const proposalService = {
    ...
    updateProposal: async (id, proposalData) => {
      try {
        ...
        try {
          const response = await api.put(`/Proposal/${id}`, updatedProposal);
          return response.data;
        } catch (error) {
    ...
    getProposalsByUser: async (userId) => {
      try {
        const response = await api.get(`/Proposal/user/${userId}`);
        return response.data;
      } catch (error) {
    ...
    getProposalsByStatus: async (status) => {
      try {
        const response = await api.get(`/Proposal/status/${status}`);
        return response.data;
      } catch (error) {
    ...
    updateProposalStatus: async (id, status) => {
      try {
        const response = await api.patch(`/Proposal/${id}/status`, { status });
        return response.data;
      } catch (error) {
    ...
    assignTeam: async (id, teamMembers) => {
      try {
        const response = await api.patch(`/Proposal/${id}/team`, { teams: teamMembers });
        return response.data;
      } catch (error) {
